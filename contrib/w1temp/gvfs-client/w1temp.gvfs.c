...
static gint timerfunc (gpointer data)
{
    ...
    {
        m->iopend = 1;
        m->source = g_file_new_for_uri(m->url);
        m->dest = g_file_new_for_path ("/tmp/.wx_static.dat");
        g_cancellable_reset(m->cancel);
	printf("<%s>\n", m->url);
        g_file_copy_async (m->source, m->dest,
